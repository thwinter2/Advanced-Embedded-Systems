...
int ServoDegreeIncrease(uint8_t Channel, uint8_t Step){
    switch(Channel){
        case SERVO_TILT_CH:
            if(ServoTiltDegree >= SERVO_TILT_MAX){
                ServoTiltDegree = SERVO_TILT_MAX;
                setServoDegree(Channel, ServoTiltDegree);
            }
            else{
                ServoTiltDegree += Step;
                setServoDegree(Channel, ServoTiltDegree); 
            }
            break;
        case SERVO_PAN_CH:
            if(ServoPanDegree >= SERVO_PAN_MAX){
                ServoPanDegree = SERVO_PAN_MAX;
                setServoDegree(Channel, ServoPanDegree); 
            }
            else{
                ServoPanDegree += Step;
                setServoDegree(Channel, ServoPanDegree); 
...
int ServoDegreeDecrease(uint8_t Channel, uint8_t Step){
    switch(Channel){
        case SERVO_TILT_CH:
            if(ServoTiltDegree <= SERVO_TILT_MIN + Step){
                ServoTiltDegree = SERVO_TILT_MIN;
                setServoDegree(Channel, ServoTiltDegree);
            }
            else{
                ServoTiltDegree -= Step;
            ...
            break;
        case SERVO_PAN_CH:
            if(ServoPanDegree <= SERVO_PAN_MIN + Step){
                ServoPanDegree = SERVO_PAN_MIN;
                setServoDegree(Channel, ServoPanDegree); 
            }
            else{
                ServoPanDegree -= Step;
...
/*
New function for ECE 785 ImStab
*/
#define SERVO_DEADZONE (50)
void Update_Servo(int error_X, int error_Y){
#if 1
  // Simple deadzone
  if (error_X > SERVO_DEADZONE)
    ServoDegreeDecrease(SERVO_PAN_CH, 1);
  else if (error_X < -SERVO_DEADZONE)
    ServoDegreeIncrease(SERVO_PAN_CH, 1);
  if (error_Y > SERVO_DEADZONE)
    ServoDegreeIncrease(SERVO_TILT_CH, 1);
  else if (error_Y < -SERVO_DEADZONE)
    ServoDegreeDecrease(SERVO_TILT_CH, 1);
#else // better controller
  static float int_error_x = 0, float int_error_y = 0;
  pos_x += error_x;
  CLIP(pos_x, 0, 180);
  setServoDegree(SERVO_PAN_CH, pos_x);
#endif
